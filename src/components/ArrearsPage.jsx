  ...
  const handleCalculateArrears = async () => {
    ...
    try {
      ...
      allWorkers.forEach(w => {
        arrearsSummary[w.worker_name] = { salary_arrears: 0, allowance_arrears: 0, bonus_arrears: 0 };
      });

      // Hissa 1 & 2: Salary & Allowance Arrears
      ...
      // Hissa 3: Bonus Arrears (NAYI AUR MUKAMMAL SAHI LOGIC)
      const fiscalYears = [];
      ...
      for (const year of fiscalYears) {
        const agreementForOldBonus = year.start.isSame(dayjs(settings.agreement_start_date), 'day') ? oldAgreement : newAgreement;
          for (const worker of allWorkers) {
              ...
              // New Bonus Package
              const newAnnualWorkEarnings = 0; // Needs detailed recalculation
  ...
  return (
    <Layout style={{ minHeight: '100vh', padding: '24px' }}>
      ...
      <Content style={{ maxWidth: '1000px', margin: '0 auto' }}>
        <Title level={2}>Arrears Calculation</Title>
        <Card>
